...
class RobotWebSocketClient:
    ...
    async def send_qr_scanned(self, order_id: int, qr_data: Dict[str, Any]):
        """发送二维码扫描结果"""
        if not self.connected or not self.websocket:
            logger.warning("WebSocket未连接，无法发送二维码扫描结果")
            return
        ...
        try:
            await self.websocket.send(json.dumps(message))
            logger.info(f"二维码扫描结果已发送: 订单 {order_id}")
        except Exception as e:
            logger.error(f"发送二维码扫描结果失败: {e}")
    
    async def send_order_update(self, order_id: int, status: str):
        """发送订单状态更新"""
        if not self.connected or not self.websocket:
            logger.warning("WebSocket未连接，无法发送订单更新")
            return
        ...
        try:
            await self.websocket.send(json.dumps(message))
            logger.info(f"订单状态更新已发送: 订单 {order_id} -> {status}")
        except Exception as e:
            logger.error(f"发送订单状态更新失败: {e}")
...
async def example_usage():
    ...
    # 创建客户端
    client = RobotWebSocketClient(SERVER_URL, ROBOT_ID, TOKEN)
