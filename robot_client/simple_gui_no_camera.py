...
class SimpleRobotGUINoCamera:
    ...
    def create_control_panel(self, parent):
        ...
        self.simulate_qr_btn = tk.Button(
            button_frame, 
            text="📱 Simulate QR Scan", 
            command=self.simulate_qr_scan,
            bg='#34C759', fg='white', font=('Arial', 12, 'bold'),
            width=20, height=2
        )
        self.simulate_qr_btn.pack(pady=5)
        
        # 快捷键提示
        ttk.Label(control_frame, text="Shortcuts: Q=Simulate QR, ESC=Exit", style='Status.TLabel').pack(pady=5)
    ...
    def simulate_qr_scan(self):
        ...
        if not self.current_orders:
            self.add_log_message("⚠️ No orders to simulate QR scan")
            return
        ...
        self.add_log_message(f"📱 Simulating QR scan for order {order_id}")
        self.gpio.beep(0.2)
        
        # 自动开门流程
        self.auto_open_door(order_id)
    ...
    def run(self):
        """运行GUI"""
        try:
            self.add_log_message("🚀 Robot GUI started successfully (No Camera Mode)")
            self.root.mainloop()
        except Exception as e:
            print(f"❌ GUI运行失败: {e}")
